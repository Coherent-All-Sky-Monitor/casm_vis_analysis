...
### Added
- `casm-layout` CLI (`layout/cli.py`): friendlier front door over the two-stage layout pipeline, with four verbs — `status` (one-line diff summary), `diff` (full position-level + wiring-row diff), `preview` (print the layout `apply` would write, per-row marked against the current one), `apply` (confirm, then regenerate `casm_wiring.csv` and the dated consumer layout CSV). Every invocation pulls the latest CAsMan DB snapshot from GitHub first (checksum-skip if unchanged; `--offline` to skip). Registered as a `_COMMANDS` entry so `run("casm-layout ...")` works from notebooks, and as a `[project.scripts]` entry point in `pyproject.toml`.
- `casm-layout preview`: renders the candidate layout as an aligned fixed-width table (one line per occupied `(snap, adc)` input, marked `+`/`~`/`-` against the current layout, with a `was:` line for changed rows), or with `-o FILE` writes the full candidate CSV (identical to what `apply` would write) without otherwise touching the layout dir. Backed by `render_preview_table` / `occupied_count` in `layout/diff.py`.
