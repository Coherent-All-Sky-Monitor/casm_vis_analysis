...
class TestAutoDetectSign:
    def test_detects_correct_sign(self):
        """Inject geometric delay with known sign, verify detection."""
        n_time = 60
        n_freq = 64
        freq_mhz = np.linspace(484.375, 390.625, n_freq)

        # Time-varying source position (transit arc) — needed so the
        # geometric delay varies across time, breaking sign degeneracy
        t = np.linspace(-0.3, 0.3, n_time)
        source_enu = np.column_stack([
            np.sin(t) * np.cos(0.3),
            np.cos(t) * np.cos(0.3),
            np.ones(n_time) * np.sin(0.3),
        ])
        ...
        detected = auto_detect_sign(vis_bl, freq_mhz, source_enu,
                                    baseline_enu)
